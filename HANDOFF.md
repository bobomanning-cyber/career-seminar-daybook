...
**Standing rule (Bob, Aug 27):** if a task starts needing extra tool calls/tokens to muscle through (a fiddly upload, a stubborn API param, repeated retries), STOP and ask Bob to do that one small step himself instead of grinding through it. Don't spend time or tokens on friction — flag it and move on. Save any real pattern/tooling problem for a dedicated planning session rather than solving it live mid-task.
...
## Session close, Aug 27
Confirmed pushed and live: `git log` shows `main` == `origin/main`, no divergence, after Bob's Code CLI push. Verified the live site's actual embedded data (not a raw HTML fetch — see the JS-rendering note below) shows all 7 days published with correct content, including Day 7's due-date framing and its slides link.

**Verification gotcha, worth remembering:** a plain URL fetch (no JS execution) of this site always looks broken — empty tables, and even the hidden `.planning-banner` text shows up on `index.html` — because the day cards/tables are rendered client-side from an embedded `const DAYS = [...]` JS blob, and the banner is only hidden via CSS (`display:none`), neither of which a non-JS fetch applies. To actually verify content, grep the embedded data directly out of the built HTML file, or use a real browser. Don't re-raise a false alarm here.

**Still open, low priority:** the Day 7 lesson-plan docx (delivered via SendUserFile this session) was never uploaded to Drive's Lessons folder — Bob is dropping it in himself rather than have this session keep retrying a fiddly `create_file` call (wrong param name, then a Google-type conversion error). Not worth automating further unless the pattern comes up again.
